...
{
	if((double)delete_/CAPACITIES[mIndex_]>=max_){
		this->resize();
		insert(p);
	}

	KeyType key_ = p.first;
	HASH_INDEX_T loc = this->probe(key_);
	if(loc == npos) throw std::logic_error("No available location");

	else if(nullptr == table_[loc])
	{
	...
	else if(key_ == (table_[loc]->item).first && (table_[loc]->deleted) == false){
		ValueType val_ = p.second;
		HashItem* change = table_[loc];
		(change->item).second = val_;
	}
...
{
	HASH_INDEX_T newp = this->probe(key);

	if((npos != newp) && nullptr != table_[newp])
	{
		table_[newp]->deleted = true;
		delete_++;
		total_--;
	}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
	...
	if(mIndex_ <= 28) 
	{
		HASH_INDEX_T size_cap = CAPACITIES[mIndex_];
		std::vector<HashItem*> table_copy(size_cap, nullptr);

		table_.resize(size_cap, nullptr);

		for(HASH_INDEX_T i = 0; i < table_copy.size(); ++i)
		{
			HashItem* newpsize = table_copy[i];

			if(newpsize == nullptr) continue;

			else if((newpsize->deleted) == false)
			{
				ItemType tempN = newpsize->item;
				insert(tempN);
				total_--;
			}

			else delete newpsize;
		}

		delete_ = 0;
	}

	else throw std::logic_error("Max Cap Reached");
